...
if __name__ == '__main__':
    ...
    parser.add_argument('input', nargs='?', default='-',
                        help='source svg file ("-" for stdin, which is default)')
    parser.add_argument('output', nargs='?',
                        help='destination svg file ("-" for stdout, skip for the same as source)')
    # choose smaller of two:
    parser.add_argument('-x', '--width',
                        help='target width (including margins, default units are mm)')
    parser.add_argument('-y', '--height',
                        help='target height (including margins, default units as mm)')
    parser.add_argument('-l', '--longest', help='target longest side')
    parser.add_argument('-s', '--shortest', help='target shortest side')
    parser.add_argument('-m', '--margin', default='0', help='margin (default units are mm)')
    parser.add_argument('-f', '--frame', action='store_true',
                        help='add white frame around the picture')
